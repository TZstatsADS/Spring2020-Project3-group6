...
model_labels = paste("GBM with n.trees =", n.trees)
```
...
if(run.cv){
  ...
  err_cv <- matrix(0, nrow = length(n.trees), ncol = 2)
  for(i in 1:length(n.trees)){
    cat("n.trees=", n.trees[i], "\n")
    err_cv[i,] <- cv.function(dat_train, K, .01, 10, n.trees[i])
...
if(run.cv){
  ...
  err_cv$n.trees = as.factor(n.trees)
  err_cv %>% 
...
```{r best_model}
if(run.cv){
  model_best <- n.trees[which.min(err_cv[,1])]
}
par_best <- list(shrinkage = .01, n.minobsinnode = 10, n.trees = model_best)
```
